...
void Data::readXml(const xml::Element& element)
{
	...
	LOG_DEBUG << "elementName is " << elementName << ends;
	    ...
	    // TODO 	I don't understand why readXml of the operator is called when there are failures? It does not make sense,
	    // 		unless some behavior should be defined at that level...
	    ...
	    LOG_DEBUG << "I finished reading an operator's parameters and childElement is " << childElement->GetText() << ends;
        }
...
void Data::writeXml(ostream& output) const
{
	...
	output<< "  </" << XML_CHILD_ELEMENT_PARAMETERS << ">" << endl;
	*/
	output << "  <" << XML_CHILD_ELEMENT_PARAMETERS << " ";
	this->_operator->writeXml(output);
	output << "/>" << endl;
    }
