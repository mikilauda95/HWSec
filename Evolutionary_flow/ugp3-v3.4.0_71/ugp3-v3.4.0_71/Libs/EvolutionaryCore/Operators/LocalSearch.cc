...
inline void LocalSearchOperator::writeXml(std::ostream& output) const
{
	output 	<< " "
		...
		<< XML_ATTRIBUTE_OUTPUT << "=\"" << this->outputPattern << "\" "; 
}
...
// - name of the output files to assimilate (with "output-individual%s.in" if more than one is produced)
	...
	if( element.ValueStr() == Data::XML_CHILD_ELEMENT_PARAMETERS )
	{
...
{
	return "" + this->getName() + " is an operator that calls an external executable to perform operations on one (or more) individual(s), written to text file(s).";
}
